...
/**
 * FreshChain - Blockchain-Based Food Traceability (Remix-ready)
 * Roles: Admin(Owner), Producer(Farmer), Transporter, Distributor, Retailer
 *
 * Flow (your agreed logic):
 * Producer creates batch (auto ID), assigns transporter
 * Transporter can log sensor data manually even before pickup (test mode)
 * Transporter accepts pickup -> IN_TRANSIT
 * Transporter transfers to Distributor
 * Distributor transfers to Retailer
 * Retailer marks arrived + inspection -> Completed
 * Customer reads full history via batchId (QR)
 *
 * Doc-required functions included:
 * registerProducer / registerTransporter / registerDistributor / registerRetailer (onlyOwner)
 * createBatch(uint batchId, string productName, uint quantity) (onlyProducer)  [also auto variant]
 * addSensorData(uint batchId, int temperature, int humidity, string location) (onlyTransporter)
 * transferOwnership(uint batchId, address newOwner) external;
 * markAsArrived(uint batchId, bool passedInspection) (onlyRetailer)
 * getBatchHistory(uint batchId) public view returns (...)
 */
...
contract FreshChain {
    ...
    // - Doc signature expects batchId param. We include it.
    // - You want auto-unique ID: use createBatchAuto().
    // -------------------------
    ...
    // -------------------------
    // Transporter: Sensor Logs (manual input)
    // - Allowed in READY_FOR_PICKUP or IN_TRANSIT, BUT only if msg.sender is assigned transporter.
    // - Temperature must be [-10..40], humidity [0..40] (Doc requirement)
    // -------------------------
    ...
    // -------------------------
    // Ownership Transfer (Doc requirement)
    // - Transporter -> Distributor
    // - Distributor  -> Retailer
    // - (Optional) Producer -> assignedTransporter (allowed but not required by your UI)
    // -------------------------
    ...
    // -------------------------
    // Retailer: Final inspection (Doc requirement)
    // -------------------------
